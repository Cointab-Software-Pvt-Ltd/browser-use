...
def test_cases() -> List[Dict[str, Any]]:
	...
	utils.logger.info(f'Loading test cases from {file_path}')

	with open(file_path, 'r') as f:
		data = json.load(f)

	subset = data[:TEST_SUBSET_SIZE]
	utils.logger.info(f'Loaded {len(subset)}/{len(data)} test cases')
	return subset
...
async def test_random_samples(test_cases: List[Dict[str, Any]], llm, context, validator):
	...
	utils.logger.info('=== Testing Random Samples ===')
	...
	for i, case in enumerate(samples, 1):
		...
		utils.logger.info(f'--- Random Sample {i}/{len(samples)} ---')
		utils.logger.info(f'Task: {task}\n')

		agent = Agent(task, llm, browser_context=context)

		await agent.run()

		utils.logger.info('Validating random sample task...')
...
def test_dataset_integrity(test_cases):
	...
	utils.logger.info(f'Checking {len(test_cases)} test cases for required fields')

	for i, case in enumerate(test_cases, 1):
		utils.logger.debug(f'Checking case {i}/{len(test_cases)}')

		for field in required_fields:
			if field not in case:
				missing_fields.append(f'Case {i}: {field}')
				utils.logger.warning(f"Missing field '{field}' in case {i}")

		# Type checks
		if not isinstance(case.get('confirmed_task'), str):
			utils.logger.error(f"Case {i}: 'confirmed_task' must be string")
			assert False, 'Task must be string'

		if not isinstance(case.get('action_reprs'), list):
			utils.logger.error(f"Case {i}: 'action_reprs' must be list")
			assert False, 'Actions must be list'

		if len(case.get('action_reprs', [])) == 0:
			utils.logger.error(f"Case {i}: 'action_reprs' must not be empty")
			assert False, 'Must have at least one action'

	if missing_fields:
		utils.logger.error('Dataset integrity check failed')
		assert False, f'Missing fields: {missing_fields}'
	else:
		utils.logger.info('✅ Dataset integrity check passed')
